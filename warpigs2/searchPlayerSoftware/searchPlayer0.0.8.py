...
class PlayerSearchApp:
    def __init__(self, root):
        ...
        adminName_frame = execute_frame = ttk.Frame(execute_frame)
        adminName_frame.pack(side=tk.LEFT, padx=50)
        ttk.Label(adminName_frame, text="Admin Name: ").pack(side=tk.LEFT)
        self.adminName_var = tk.StringVar()
        self.adminName_entry = ttk.Entry(adminName_frame, textvariable=self.adminName_var, width=30)
        self.adminName_entry.pack(side=tk.LEFT)
...
© 2025 by Reclaim Development LLC. All rights reserved."""
    ...
    def perform_search(self):
        ...
        try:
            with psycopg2.connect(**{
                "dbname": "armaReforger",
                "user": "search2test",
                "password": "adsioj1098234u9joasdf098u10349ui103j018923u",
    ...
    def execute_command(self):
        ...
        if not self.adminName_var.get():
            messagebox.showerror("Execution Error", "Enter your name into the admin name section")
            return
    ...
    def postInDiscord(self):
        if self.command_type_var.get() == "ban":
            ...
            ban_title = f"Banned: {self.selected_name.replace("_", "\\_").replace("*", "\\*")}"
            ban_description = (
                f"**Ban Reason:** {self.reason_var.get()}\n"
                + f"**Banned on:** {datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%d %H-%M")} UTC\n"
                + f"**Ban Duration:** {ban_duration_str}\n"
                + f"**Banned By:** {self.adminName_var.get()}"
            )
            ...
            ban_title = f"{self.selected_name.replace("_", "\\_").replace("*", "\\*")} Unbanned"
            ban_description = (
                f"**Unbanned Reason:** {self.reason_var.get()}\n"
                + f"**Unbanned on:** {datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%d %H-%M")} UTC\n"
                + f"**Unbanned by:** {self.adminName_var.get()}"
            )
            response = requests.post(
                "https://discord.com/api/webhooks/1361087913772843099/PRZMSrAaojV0xoNFBY6oDvnQV6I5Itg06sOD6VjYp4aKXqBHSH1Mg2WokYgdgy2kdUqZ",
                json={
