...
class PlayerSearchApp:
    def __init__(self, root):
        ...
        # 3. Command Section
        command_frame = ttk.LabelFrame(main_frame, text="Ban Command", padding="10")
        command_frame.pack(fill=tk.X, pady=10)
        
        # First row - Command preview and copy button
        ...
        ttk.Label(time_frame, text="Ban Time (numbers only):").pack(anchor=tk.W)
        vcmd_num = (self.root.register(self.validate_number), '%P')
        self.time_var = tk.StringVar()
        time_entry = ttk.Entry(time_frame, textvariable=self.time_var, validate="key", validatecommand=vcmd_num)
        ...
        ttk.Label(reason_frame, text="Ban Reason:").pack(anchor=tk.W)
    ...
    def copy_command(self):
        ...
        try:
            ...
            if command == "#ban create IDENTITY":
                raise Exception("Please select a player to ban")
    ...
    def perform_search(self):
        ...
        if not search_term:
            messagebox.showwarning("Invalid Input", "Please select a player to ban")
            return
